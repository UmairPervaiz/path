...
class SearchController extends Controller
{
    ...
    public function jobs(Request $request)
    {
        if ($request->country_id != null && $request->industry_id != null && $request->company_name != null)
        {
            ...
            $jobs = DB::table('jobs')
                ->select( 'jobs.id as job_id','jobs.job_location as job_location','jobs.title as title',
                    'jobs.description as description','jobs.date as date','jobs.expireDate as expireDate',
                    'jobs.category as category','users.avatar as avatar','users.category as user_category')
                ->join('users','jobs.user_id','=','users.id')
                ->where('jobs.status', '=',1)
                ->where('jobs.approval_status', '=',1)
                ->whereDate('jobs.expireDate','>=', Carbon::now())
                ->where('jobs.job_location', '=',$request->country_id)
                ->where('jobs.category','=', $request->industry_id)
                ->where('users.name','=', $request->company_name)
